...
def main(data_num, viz):
    ...
    for i in opt_leaves:
        ...
        sdf_leaf.append(leafs[centroids[i][1], centroids[i][0], 3].astype('uint8'))

    leafs_ = np.zeros(shape=(1080,1440))
    for leaf in sdf_leaf:
        leafs_ += leafs[:,:,3] == leaf
    leafs[:,:,3] *= leafs_
