...
pcd_path = "PCDandDepth/Pointclouds/demo_ (1).pcd"
...
mask_image = Image.open('PCDandDepth/Images/aggrigated_masks (1).png')
...
depth = np.load("PCDandDepth/Arrays/depth (1).npy")
...
color_index = np.reshape(color_index, (1080, 1440, 1))
masked_points = np.concatenate((xy_pos_masked,depth_masked,color_index), axis=2)

left_image = Image.open("PCDandDepth/Images/left_rect (1).png")
